...
png(filename="Plots//Mintail Exp EM R0 Sel SSB Plots.png", width=7, height=5, units="in", res=200)
ggplot(QRCA2, aes(x=Year)) +
  ...
  scale_color_manual(values=rev(c("#08306b",
                                  ...
                                  "#54278f",
                                  "#3f007d"))) +
  labs(y=c("SSB"), title = "Mintail SSB for all MSE Sims") +
...
png(filename="Plots//Mintail Exp EM R0 Sel Depl Plots.png", width=7, height=5, units="in", res=200)
...
ggplot(QDepl_OM2, aes(x=Year)) +
  ...
  labs(y=c("Depletion"), title = "Mintail Depl for all MSE Sims") +
  ylim(0, 1.1) +
  theme(strip.text = element_text(size=5))
