...
Dirn <- "E:/ratpacktest/highR0/mintail/estR0_estSel/"
Scen <- Scenario[1]
...
# HighR0 SelMidPt FixR FixSel ####
Dirn <- "E:/ratpacktest/highR0/mintail/fixR0_fixSel/"
Scen <- Scenario[1]
...
# LowR0 SelMidPt EstR EstSel ####
Dirn <- "E:/ratpacktest/lowR0/mintail/estR0_estSel/"
Scen <- Scenario[2]
R0_status <- R0Status[1]
Sel_status <- SelStatus[1]
...
# LowR0 SelMidPt FixR FixSel ####
Dirn <- "E:/ratpacktest/lowR0/mintail/fixR0_fixSel/"
Scen <- Scenario[2]
R0_status <- R0Status[2]
Sel_status <- SelStatus[2]
...
QSel <- QSel %>%
  mutate(OMSelex=OMSelex)

QSel2 <- QSel %>%
  mutate(ParStatus = paste(Scenario, R0Status, SelStatus, sep = " "))

png(filename="Plots//Mintail EM R0 Sel Sel Curve Plots.png", width=7, height=5, units="in", res=200)

ggplot(QSel2, aes(x=LengthBin, group=ParStatus)) + 
  ...
  labs(title = "Mintail Selectivity by Length Bin for all MSE Sims", x = "Length Bin", y = "Selectivity") +
